...
const MapView = React.createClass({
    ...
    switch (Platform.OS) {
      case 'android':
        NativeModules.UIManager.dispatchViewManagerCommand(
          this._getHandle(),
          NativeModules.UIManager.AIRGoogleMap.Commands[name],
          args
        );
        break;

      case 'ios':
        NativeModules.AIRGoogleMapManager[name].apply(
          NativeModules.AIRGoogleMapManager[name],
  ...
  render() {
    ...
    return (
      <AIRGoogleMap ref="map" {...props} />
    );
  },
});

let AIRGoogleMap = requireNativeComponent('AIRGoogleMap', MapView, {
  nativeOnly: {
    onChange: true,
    onMapReady: true,
    handlePanDrag: true,
